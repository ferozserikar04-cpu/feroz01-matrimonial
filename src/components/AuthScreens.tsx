...
export const AuthScreens: React.FC<AuthScreensProps> = ({
  ...
  useEffect(() => {
    // Helper function to remove any lingering grecaptcha badges from the DOM
    const cleanupBadgesAndVars = () => {
      try {
    ...
    // Only initialize when currentScreen is 'login' and loginMethod is 'phone'
    if (currentScreen !== 'login' || loginMethod !== 'phone') {
      // Clean up if navigating away
      if (recaptchaVerifierRef.current) {
        try {
          recaptchaVerifierRef.current.clear();
        } catch (e) {
          console.error('Error clearing recaptcha verifier on state change:', e);
        }
        recaptchaVerifierRef.current = null;
      }
      cleanupBadgesAndVars();
      return;
    }

    let isSubscribed = true;

    // Give react time to render the 'recaptcha-container' DOM element
    const timer = setTimeout(() => {
      ...
      // If we already have one, clear it first
      if (recaptchaVerifierRef.current) {
        try {
          recaptchaVerifierRef.current.clear();
        } catch (e) {}
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    if (loginMethod === 'email') {
      ...
      try {
        ...
        // Get the active verifier (prioritize useEffect-managed ref)
        let appVerifier = recaptchaVerifierRef.current || (window as any).recaptchaVerifier;
        if (!appVerifier) {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        const googleProfile = {
          ...
          email: user.email || '',
          gender: 'Female' as const, // default to Female for matching with Feroz (Male)
          age: 25,
          height: "5'4\"",
          religion: 'Islam',
          caste: 'General',
          ...
          education: 'Graduate',
          location: 'Delhi NCR, India',
          photos: [user.photoURL || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=500&auto=format&fit=crop&q=80'],
          avatar: user.photoURL || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=150&auto=format&fit=crop&q=80',
          verified: true,
          online: true,
          matchScore: 92,
        ...
        <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
          ...
          <div className="space-y-4 mb-4">
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 bg-white border border-pink-200 hover:bg-pink-50/30 text-gray-700 font-semibold py-3 px-6 rounded-full shadow-sm transition transform active:scale-[0.98] disabled:opacity-60"
            >
          ...
          <div className="flex items-center my-4">
            ...
            <span className="px-3 text-[10px] text-gray-400 uppercase font-mono font-bold tracking-widest">Or login with Email</span>
            <div className="flex-1 border-t border-pink-100"></div>
          </div>
